...
"""
FIXED: FikFap Video Downloader and Organizer - CDN Authentication Support

This module downloads videos from M3U8 playlists with proper CDN authentication
and browser-like headers to avoid 403 errors from BunnyCDN.
"""
...
class VideoDownloaderOrganizer:
    """Complete video downloader and organizer for FikFap posts with CDN auth support"""
    ...
    async def download_and_organize_post(self, post_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            playlist_result = await self._download_main_playlist(video_stream_url, m3u8_dir, post_data)
            ...
            qualities_result = await self._download_all_qualities(
    ...
    async def _download_main_playlist(self, video_stream_url: str, m3u8_dir: Path, post_data: Dict[str, Any]) -> Dict[str, Any]:
        """Download and parse the main M3U8 playlist with enhanced CDN authentication"""
        try:
            print(f"📡 Downloading main playlist from CDN...")
            ...
            print(f"🔐 Using enhanced CDN authentication headers...")
    ...
    async def _download_all_qualities(
        self, 
        ...
        for quality in qualities:
            try:
                ...
                # Download the quality playlist and segments
                result = await self._download_quality_variant(quality, quality_dir, base_url, post_data)
    ...
    async def _download_quality_variant(
        self, 
                ...
                try:
                    ...
                    async with self.session.get(segment_url, headers=request_headers) as response:
                        if response.status == 200:
                            ...
                            print(f"⚠️ Failed to download segment {i}: HTTP {response.status}")
